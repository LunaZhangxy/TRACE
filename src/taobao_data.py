...
def get_taobao_data_np(debug=False):
    _local_path = "xx/Taobao_UserBehavior.csv"
    df = pd.read_csv(_local_path)
    df = df.rename(columns={
        df.columns[0]: "user_id",
        df.columns[1]: "item_id",
        df.columns[2]: "item_category",
        df.columns[3]: "click_ts",
        df.columns[4]: "pay_ts",
        df.columns[5]: "cart_ts",
        df.columns[6]: "fav_ts",
    })
   

    df[["pay_ts", "cart_ts", "fav_ts"]] = (
        df[["pay_ts", "cart_ts", "fav_ts"]]
        .fillna(-1)
        .astype(np.int64)
    )

    df[["user_id", "item_id", "item_category", "click_ts"]] = (
        df[["user_id", "item_id", "item_category", "click_ts"]]
        .astype(np.int64)
    )

    # user_id, item_id, item_cate, click_ts, pay_ts, cart_ts, fav_ts
    res = df[["user_id", "item_id", "item_category",
              "click_ts", "pay_ts", "cart_ts", "fav_ts"]].to_numpy()
